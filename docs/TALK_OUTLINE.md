...
6. Limitations (1 min). Two symbols; source missing 36% of sessions; EOD only; continuous-yield dividend approximation; the mids that violate no-arbitrage bounds under near-zero rates.
   ...
   Added 2026-09-17 (Amendment 8, exploratory): every comparison prices American contracts with the European formula; the early-exercise premium averages 0.40% of the mid on set A (puts 0.70%, calls 0.03%; 32% of quotes above the tick, 35% from 2022 onward against 14% in 2020-21). The consistent tree treatment moves the M(RV)-versus-B1 gap by 2%, so the European engine favors B1 only slightly; on below-tick rows M(RV) versus B1 stays "baseline wins" while the fragile M(B1) label drops to "no evidence either way". Every interval is conditional on the fitted models: the bootstrap resamples evaluation sessions only while adjacent folds share 249 of 250 training sessions, so total uncertainty is larger than reported.
